...
export class ResetPasswordComponent {
  ...
  calculatePasswordStrength() {
    ...
    if (score <= 2) {
      this.passwordStrength = {
        ...
        text:
          'Contraseña débil' +
          (feedback.length ? ': ' + feedback.slice(0, 2).join(', ') : ''),
        score,
      };
    } else if (score <= 4) {
      this.passwordStrength = {
        ...
        text:
          'Contraseña regular' +
          (feedback.length ? ': ' + feedback.slice(0, 1).join(', ') : ''),
        score,
      };
    } else if (score <= 5) {
      this.passwordStrength = {
  ...
  validateForm(): boolean {
    ...
    if (this.passwordStrength.score < 3) {
      this.showMessage(
        'La contraseña es muy débil. Por favor, use una contraseña más segura',
        'error'
      );
      return false;
    }
  ...
  resetPassword() {
    ...
    // llamada HTTP
    this.http
      .put<any[]>(`https://ucvbotbackend.onrender.com/api/admins/reset`, {
